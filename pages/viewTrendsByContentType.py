...
# Rename columns to align with expected structure
df.rename(columns={
    'Date': 'Date Posted',
    'views': 'Views',
    'likes': 'Likes',
...
# Parse dates and filter invalid entries
...
with col2:
    start_month = st.selectbox("Start Month", months, index=0, format_func=lambda x: calendar.month_name[x])
with col3:
    end_year = st.selectbox("End Year", years, index=len(years) - 1)
with col4:
    end_month = st.selectbox("End Month", months, index=11, format_func=lambda x: calendar.month_name[x])
...
df_filtered = df[(df['Date Posted'] >= start_date) & (df['Date Posted'] <= end_date)]

# Prepare month column after filtering
df_filtered['Month'] = df_filtered['Date Posted'].dt.to_period('M').dt.to_timestamp()
...
fig_multi = px.line(
    ...
    y='Views',
    color='Content Type',
    labels={'Views': 'Total Views', 'Month': 'Month', 'Content Type': 'Content Type'},
...
video_counts = df_filtered.groupby('Content Type')['Video ID'].count().reset_index()
...
fig_bar = px.bar(
    ...
    x='Content Type',
    y='Total Videos',
    labels={'Total Videos': 'Number of Videos', 'Content Type': 'Content Type'},
    template='plotly_dark',
...
total_views_by_type = df_filtered.groupby('Content Type')['Views'].sum().reset_index()
...
selected_content = st.selectbox("Select Content Type for Trend Analysis", options=sorted(content_types))

# Filter data for selected content type
filtered_df = df_filtered[df_filtered['Content Type'] == selected_content]

if filtered_df.empty:
    st.warning("No data available for the selected content type in the chosen date range.")
    st.stop()
...
fig_views = px.line(
    ...
    x='Month',
    y='Views',
    title=f"Monthly Total Views for '{selected_content}' Content",
...
for _, video in top_videos.iterrows():
    video_url = video['Video ID'] if video['Video ID'].startswith('http') else f"https://www.youtube.com/watch?v={video['Video ID']}"
