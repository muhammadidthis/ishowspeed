...
st.markdown("### 📊 Sentiment per Video ID (Click to View Video)")
...
fig_bar = px.histogram(
    ...
    color="sentiment",
    barmode="group",
    title="Sentiment Counts by Video ID",
    text_auto=True,
...
# Capture click event
click_result = plotly_events(fig_bar, click_event=True, select_event=False)
...
if click_result:
    clicked_video_id = click_result[0]["x"]
    st.markdown("### 🎬 YouTube Video")
    st.video(f"https://www.youtube.com/watch?v={clicked_video_id}")
